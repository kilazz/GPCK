...
{
    uint32_t maskVal = (1u << bitsLength) - 1u;
    return (value >> bitsOffset) & maskVal;
}
...
struct TileStream
{
    ...
    static TileStream construct(uint32_t streamInPos, in ByteAddressBuffer inputBuf)
    {
        TileStream ts;

        ts.m_word1 = inputBuf.Load(streamInPos);
        ts.m_word2 = inputBuf.Load(streamInPos + 4);
    ...
    TileParams GetTileParams(uint32_t streamInPos, uint32_t streamOutPos, uint32_t tileIdx, in ByteAddressBuffer inputBuf)
    {
        TileParams params;

        const uint32_t tileTablePos = streamInPos + kStreamHeaderSize;

        params.inPos = (tileIdx > 0 ? inputBuf.Load(tileTablePos + tileIdx * 4) : 0);

        if (tileIdx == m_numTiles - 1)
            params.inSize = inputBuf.Load(tileTablePos);
        else
            params.inSize = inputBuf.Load(tileTablePos + (tileIdx + 1) * 4) - params.inPos;
